...
while True:
    ...
    if virtual_amount >= 94000:
        today_balancing_count += 1
        # bot.sendMessage(chat_id=chat_id, text=f"Need Balancing:{today_balancing_count}")
        virtual_amount = 0

    if ub_spread >= 1:
        if upbit_ask_quantity > bithumb_bid_quantity:
            virtual_amount += bithumb_bid_quantity / 3
        elif bithumb_bid_quantity > upbit_ask_quantity:
            virtual_amount += upbit_ask_quantity / 3
    ...
    if uc_spread >= 1.3:
        if upbit_bid_quantity > coinone_ask_quantity:
            virtual_amount += coinone_ask_quantity / 3
